...
/**
 ...
 @modification on 2009-02-02 to implement 2GB+ file support by Lord Alderaan (lordalderaan@gmail.com)
 @example
...
class httpdownload {
	 ...
	 **/
	function header($size=null,$seek_start=null,$seek_end=null) {
	...
	function download() {
		...
		{
			$size = sprintf("%u", filesize($this->data));
			...
			if (ftell($res)==0) {
				...
				rewind($res);
				$this->header();
				while (!(connection_aborted() || connection_status() == 1) && feof($res) == false)
				{
